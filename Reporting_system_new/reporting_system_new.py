...
Source trades table: core.trades  (new DB)
Targets under: reports schema (eod_positions, fetched_eod_prices, pnl_daily)

Implements:
- Ticket-scoped FIFO (partition includes trade_number)
- EOD marking (best mark by latest retrieved_at)
- Auto-close after expiry at expiry-day mark
- Daily aggregation into reports.pnl_daily
- No fees anywhere
- QA summary counters printed at the end
- NEW: enqueue open positions into reports.fetched_eod_prices as 'PENDING'
"""
...
# -------------------------
# HARD-CODED DB URL (keep as URL object)
# -------------------------
REPORT_DB_URL = URL.create(
    drivername="postgresql+psycopg2",
    username="postgres",
    password="New@1234",   # '@' is fine here
    host="192.168.18.23",
    port=5432,
    database="postgres",   # DB name you confirmed
)
...
class Config:
    start_date: date
    end_date: date
    src_schema: str = "core"       # source: core.trades
    src_table: str = "trades"
    rpt_schema: str = "reports"
    db_url: URL = REPORT_DB_URL    # keep URL type, not str
...
def get_conn(db_url: URL):
    ...
    try:
        return eng.begin()  # transactional context manager
...
def sql_select_trades(cfg: Config) -> str:
    # trade_time is a timestamptz; trade_day parsed from trade_date text
    return f"""
select
  t.account_id,
  t.strategy_id,
  ...
  t.user_id,
  t.instrument_name,
  upper((t.type)::text)             as option_type,    -- CE/PE from enum/domain
  (t.strike)::numeric               as strike,
  (t.expiry_date)::date             as expiry_date,
  (t.trade_number)::bigint          as trade_number,
  t.trade_time                      as trade_ts,       -- use directly
  to_date((t.trade_date)::text,'YYYY-MM-DD') as trade_day,
  (t.qty)::numeric                  as qty,
  (t.trade_price)::numeric          as trade_price,
  concat_ws('||',
    t.account_id, t.strategy_id, coalesce(t.strategy_variant_id,0), t.user_id,
    t.instrument_name, upper((t.type)::text),
    coalesce((t.strike)::text,''), coalesce((t.expiry_date)::text,''), coalesce((t.trade_number)::text,'0')
...
insert into "{cfg.rpt_schema}"."fifo_inventory_today" as f(
  position_key_trade, account_id, strategy_id, strategy_variant_id, user_id,
  instrument_name, option_type, strike, expiry_date, trade_number,
  trade_day, eod_net_qty, avg_cost_fifo, realized_pnl_fifo_today
)
values (
  :position_key_trade, :account_id, :strategy_id, :strategy_variant_id, :user_id,
  :instrument_name, :option_type, :strike, :expiry_date, :trade_number,
...
def sql_best_mark(cfg: Config) -> str:
    ...
    where date=:day and instrument_name=:instr and option_type=:otype
      and strike=:strike and expiry_date=:expiry
...
def sql_best_expiry_close(cfg: Config) -> str:
    ...
    select price
    from "{cfg.rpt_schema}"."fetched_eod_prices"
    where instrument_name=:instr and option_type=:otype and strike=:strike and expiry_date=:expiry
      and date = :expiry
    order by retrieved_at desc
    limit 1
    """
...
insert into "{cfg.rpt_schema}"."eod_positions" as t(
  trade_day, account_id, strategy_id, strategy_variant_id, user_id,
  instrument_name, option_type, strike, expiry_date,
  eod_net_qty, avg_cost_fifo, eod_price, mv_eod,
  realized_pnl_fifo_today, carry_in, prev_eod_qty, prev_mv_eod
)
values (
  :trade_day, :account_id, :strategy_id, :strategy_variant_id, :user_id,
  :instrument_name, :option_type, :strike, :expiry_date,
...
insert into "{cfg.rpt_schema}"."pnl_daily" as d(
  trade_day, account_id, strategy_id, strategy_variant_id, user_id,
  realized_cash_today, unrealized_change, total_pnl_day, cum_total_pnl
)
select
  :day as trade_day, account_id, strategy_id, strategy_variant_id, user_id,
...
# ---- QA SQL helpers for unparsable trade_date ----
...
# -------------------------
# ENQUEUE step: add instruments needing EOD prices (PENDING)
# -------------------------
def sql_enqueue_eod_marks(cfg: Config) -> str:
    return f"""
    INSERT INTO "{cfg.rpt_schema}"."fetched_eod_prices"
        (date, instrument_name, option_type, strike, expiry_date, price, retrieved_at)
    SELECT DISTINCT
...
# -------------------------
# EOD join + auto-close + P&L (with QA counts)
# -------------------------
def get_best_mark(conn: Connection, cfg: Config, day: date, instr: str, otype: str, strike: float, expiry: date) -> Optional[float]:
    row = run_sql(conn, sql_best_mark(cfg), day=day, instr=instr, otype=otype, strike=strike, expiry=expiry).fetchone()
    return None if row is None else row[0]

def get_best_expiry_close(conn: Connection, cfg: Config, instr: str, otype: str, strike: float, expiry: date) -> Optional[float]:
    row = run_sql(conn, sql_best_expiry_close(cfg), instr=instr, otype=otype, strike=strike, expiry=expiry).fetchone()
    if row and row[0] is not None:
        return row[0]
    row = run_sql(conn, sql_best_expiry_fallback(cfg), instr=instr, otype=otype, strike=strike, expiry=expiry).fetchone()
    return None if row is None else row[0]

def build_eod_positions_for_day(conn: Connection, cfg: Config, day: date) -> int:
    """Returns count of keys missing EOD marks for this day."""
    missing_marks = 0
    ...
    for r in fifo_rows:
        eod_price = get_best_mark(conn, cfg, day, r["instrument_name"], r["option_type"], float(r["strike"]), r["expiry_date"])
        ...
        prev = run_sql(conn, f"""
            ...
            from "{cfg.rpt_schema}"."eod_positions"
            where account_id=:a and strategy_id=:s and strategy_variant_id is not distinct from :sv
              and user_id is not distinct from :u
              and instrument_name=:i and option_type=:o and strike=:k and expiry_date=:e
              and trade_day = (
                select max(trade_day) from "{cfg.rpt_schema}"."eod_positions"
                where account_id=:a and strategy_id=:s and strategy_variant_id is not distinct from :sv
                  and user_id is not distinct from :u
                  and instrument_name=:i and option_type=:o and strike=:k and expiry_date=:e
                  and trade_day < :day
              )
        """, a=r["account_id"], s=r["strategy_id"], sv=r["strategy_variant_id"], u=r["user_id"],
             i=r["instrument_name"], o=r["option_type"], k=r["strike"], e=r["expiry_date"], day=day).fetchone()
...
def apply_autoclose_after_expiry(conn: Connection, cfg: Config, day: date) -> int:
    """Returns count of rows auto-closed on this day."""
    rows = run_sql(conn, f"""
        select account_id, strategy_id, strategy_variant_id, user_id,
               instrument_name, option_type, strike, expiry_date,
               eod_net_qty, avg_cost_fifo
        from "{cfg.rpt_schema}"."eod_positions"
        where trade_day=:day and eod_net_qty <> 0 and trade_day > expiry_date
    """, day=day).mappings().all()

    applied = 0

    for r in rows:
        expiry_px = get_best_expiry_close(conn, cfg, r["instrument_name"], r["option_type"],
                                          float(r["strike"]), r["expiry_date"])
        if expiry_px is None:
            continue

        qty = float(r["eod_net_qty"])
        ac  = float(r["avg_cost_fifo"] or 0.0)
        realized_bump = (expiry_px - ac) * qty if qty > 0 else (ac - expiry_px) * abs(qty)
        ...
        run_sql(conn, f"""
            update "{cfg.rpt_schema}"."eod_positions"
               set realized_pnl_fifo_today = coalesce(realized_pnl_fifo_today,0) + :add_real,
                   ...
                   avg_cost_fifo = null
             where trade_day=:day
               and account_id=:a and strategy_id=:s and strategy_variant_id is not distinct from :sv
               and user_id is not distinct from :u
               and instrument_name=:i and option_type=:o and strike=:k and expiry_date=:e
        """, add_real=realized_bump, px=expiry_px, day=day,
             a=r["account_id"], s=r["strategy_id"], sv=r["strategy_variant_id"], u=r["user_id"],
             i=r["instrument_name"], o=r["option_type"], k=r["strike"], e=r["expiry_date"])
...
def run_pipeline(cfg: Config):
    ...
    with get_conn(cfg.db_url) as conn:
        ...
        # 1) FIFO per ticket-scoped bucket
        rows = run_sql(conn, sql_select_trades(cfg),
                       d_from=cfg.start_date, d_to=cfg.end_date).mappings().all()
        ...
        # 2) Day loop: enqueue → build positions → autoclose → aggregate
        cur = cfg.start_date
        while cur <= cfg.end_date:
            # NEW: add PENDING fetch requests for open positions of the day
            added = enqueue_eod_marks(conn, cfg, cur)
            # print(f"[ENQUEUE] {cur} → {added} new requests")  # optional
    ...
    print(f"Unparsable trade_date rows : total={qa.unparsable_trade_date_total} | in_window(heuristic)={qa.unparsable_trade_date_in_window}")
