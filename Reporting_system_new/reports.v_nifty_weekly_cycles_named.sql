-- reports.v_nifty_weekly_cycles_named source

CREATE OR REPLACE VIEW reports.v_nifty_weekly_cycles_named
AS WITH daily AS (
         SELECT v_pnl_daily_cycles_named.trade_day,
            v_pnl_daily_cycles_named.account_id,
            v_pnl_daily_cycles_named.strategy_id,
            v_pnl_daily_cycles_named.strategy_name,
            v_pnl_daily_cycles_named.strategy_variant_id,
            v_pnl_daily_cycles_named.strategy_variant_name,
            v_pnl_daily_cycles_named.user_id,
            v_pnl_daily_cycles_named.user_name,
            v_pnl_daily_cycles_named.instrument,
            v_pnl_daily_cycles_named.realized_cash_today,
            v_pnl_daily_cycles_named.unrealized_change,
            v_pnl_daily_cycles_named.total_pnl_day
           FROM reports.v_pnl_daily_cycles_named
          WHERE v_pnl_daily_cycles_named.instrument = 'NIFTY'::text
        ), wk AS (
         SELECT index_week_cycles.instrument,
          ...
          WHERE index_week_cycles.instrument = 'NIFTY'::text
        ), mn AS (
         SELECT index_month_cycles.instrument,
          ...
          WHERE index_month_cycles.instrument = 'NIFTY'::text
        ), daily_in_week AS (
         SELECT d.trade_day,
            d.account_id,
            d.strategy_id,
 ...
 SELECT j.instrument,
    ...
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 3::numeric THEN j.realized_cash_today
            ELSE 0::numeric
        END) AS wed_realized,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 3::numeric THEN j.unrealized_change
            ELSE 0::numeric
        END) AS wed_unrealized_change,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 3::numeric THEN j.total_pnl_day
            ELSE 0::numeric
        END) AS wed_total,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 4::numeric THEN j.realized_cash_today
            ELSE 0::numeric
        END) AS thu_realized,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 4::numeric THEN j.unrealized_change
            ELSE 0::numeric
        END) AS thu_unrealized_change,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 4::numeric THEN j.total_pnl_day
            ELSE 0::numeric
        END) AS thu_total,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 5::numeric THEN j.realized_cash_today
            ELSE 0::numeric
        END) AS fri_realized,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 5::numeric THEN j.unrealized_change
            ELSE 0::numeric
        END) AS fri_unrealized_change,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 5::numeric THEN j.total_pnl_day
            ELSE 0::numeric
        END) AS fri_total,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 1::numeric THEN j.realized_cash_today
            ELSE 0::numeric
        END) AS mon_realized,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 1::numeric THEN j.unrealized_change
            ELSE 0::numeric
        END) AS mon_unrealized_change,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 1::numeric THEN j.total_pnl_day
            ELSE 0::numeric
        END) AS mon_total,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 2::numeric THEN j.realized_cash_today
            ELSE 0::numeric
        END) AS tue_realized,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 2::numeric THEN j.unrealized_change
            ELSE 0::numeric
        END) AS tue_unrealized_change,
    sum(
        CASE
            WHEN EXTRACT(isodow FROM j.trade_day) = 2::numeric THEN j.total_pnl_day
            ELSE 0::numeric
        END) AS tue_total
